...
/*
32 sdr, 22 sr
too many segments, it makes more sense to use visualization to do the EDA for this part
*/
select sdr_id, count(*)
from closed_deals
group by sdr_id
...
# sdr by sr
select sdr_id, sr_id, count(*)
from closed_deals
group by sdr_id, sr_id
order by count(*) desc
;

# sdr by sr by business_segment
select sdr_id, sr_id, business_segment, count(*)
from closed_deals
group by sdr_id, sr_id, business_segment
order by count(*) desc
;

# sdr by sr by lead_type
select sdr_id, sr_id, lead_type, count(*)
from closed_deals
group by sdr_id, sr_id, lead_type
order by count(*) desc
;

# sdr by sr by lead_behaviour_profile
select sdr_id, sr_id, lead_behaviour_profile, count(*)
from closed_deals
group by sdr_id, sr_id, lead_behaviour_profile
order by count(*) desc
;
